...
public class GuardPatrol : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        enemyAIController = gameObject.GetComponent<GuardAI>();
    ...
    void Update()
    {
        ...
        while (isAlerted == true)
        {
            ...
            {
                                                //Note - this block was commended outon 06/07/2020 to improve the chase
...
//
    ...
    public void IsRunningToTheShouting(GameObject WhoShouted)
    {
        ...
        while (isRunningToShout == true)
        {
            Startled = false;
            // waiting once we have reached a location close to the target position
            if (Vector3.Distance(transform.position, WhoShouted.transform.position) < 1.9f)
            {
                Jogging = false;
                Walking = false;
                PauseInWalk = true;
