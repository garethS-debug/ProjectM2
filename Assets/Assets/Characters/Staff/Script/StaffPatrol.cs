...
public class StaffPatrol : MonoBehaviour
{
   ...
   // private WaypointInfo waypointInfo;
    ...
    public Transform standViewLocation;
    public Transform resetFilingPOS;
    ...
    void Start()
    {
        ...
        staffAIController = gameObject.GetComponent<StaffAIController>();
    ...
    void Update()
    {
        ...
        if (playerPOS <= staffFOV.alertRange && staffFOV.inPeriphVision == true)
        {
        ...
        if (staffAIController.State2 == true && playerPOS <= staffFOV.alertDistance)
        {
            inAlertRange = true;
        }
        ...
        while (isOnPatrol == true && isRunningToFood == false)
        {
            ...
            if (Vector3.Distance(transform.position, moveLocations[randomSpot].position) <= 0.7f || isSitting == true || isFiling == true)
            {
                Walking = false;
                PauseInWalk = true;
                ...
                if (waitTime < 0)
                {
                    ...
                    {
                        isFiling = false;
                      //  this.transform.position = resetFilingPOS.position;
                    }
        ...
        while (isWalkingToDistraction == true)
        {
            ...
            if (distraction.gameObject != null)
            {
                ...
                if (Vector3.Distance(transform.position, distraction.gameObject.transform.position) > 2.0f)
                {
                ...
                if (Vector3.Distance(transform.position, distraction.gameObject.transform.position) <= 2.0f)
                {
                    ...
                    if (staffAIController.IveSeenTheDistraction == true)
                    {
                        staffAIController.DistractionTimers();
                    }
        ...
        while (isWalkingToLastKnownLOC == true)
        {
            ...
            while (Vector3.Distance(transform.position, LastKnownGuardLocation) < 1.0f)
            {
              ...
              //  Alarm.Instance.GuardsAlertedByStaff(staffFOV.LastKnownMurphyLOC);
                ...
                guardAIComponent.StaffAlertingGuard = true;
        ...
        while (staffAIController.State2 == true && staffFOV.PlayerinFOV == true)
        {
        ...
        if (Jogging == true)
        {
            ...
            // print(angle);
            anim.SetFloat("Angle", angle, speedDampTime, Time.deltaTime);
            anim.SetFloat("Speed", 0.7f, speedDampTime, Time.deltaTime);
          //  anim.SetFloat("Direction", direction, DirectionDampTime, Time.deltaTime);
            //Direction -- > Anim
         //   direction = angle / 360;
        ...
        if (isFiling == true)
        {
          ...
          //  this.transform.position = standLocation.position;
            transform.LookAt(standViewLocation);

            if (waitTime > 0.5f)
            {
                anim.SetBool("isFiling", true);
            }
    ...
    //   print("Waypoint:" + other.name);
        ...
        if (other.gameObject.tag == "PatrolPoint")
        {
            ...
            //Disable Bool
            if (other.gameObject.name == "FilingCabinet")
            {
                waitTime = other.gameObject.GetComponent<WaypointInfo>().WaypointWaitTime;
                isFiling = true;
            }
