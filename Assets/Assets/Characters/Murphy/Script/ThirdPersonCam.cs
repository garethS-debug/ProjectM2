...
public class ThirdPersonCam : MonoBehaviour
{
    ...
    [Header("Focus Cam")]
    public KeyCode FocusCam = KeyCode.LeftShift;
    //// Inspector serialized	
    //[SerializeField]
    //private Transform cameraXform;
    [SerializeField]
    private float distanceAway; //Fixed distance away from character
    //[SerializeField]
    //private float distanceAwayMultipler = 1.5f;
    [SerializeField]
    private float distanceUp; //Fixed distance up from character
    //[SerializeField]
...
//    private BarsEffect barsEffect;
    private CamStates camState = CamStates.Behind; // defeult to behind
  ...
  //  private CamStates startingState = CamStates.Free;
    ...
    private void LateUpdate()               //Camera update so eerything is in position
    {
        //Distance from the characters transform + hight. 
        Vector3 characterOffset = followXform.position + offset;

        if (Input.GetKey(FocusCam))
        {
        ...
        switch (camState)
        {
            case CamStates.Behind:
               ...
               // CinematicBars.Instance.HideBars();
                ...
                //calculate direction camera is from the player, kill y, and normalise to give a valid direction with unit magnatude 
                lookDirection = characterOffset - this.transform.position;
                lookDirection.y = 0;
                lookDirection.Normalize();
                Debug.DrawRay(this.transform.position, lookDirection, Color.green); // draw ray from camera. 
                ...
                Debug.DrawRay(followXform.position, -1f * followXform.forward * distanceAway, Color.blue); //Draw Character pos
                Debug.DrawRay(followXform.position, targetPosition, Color.magenta);                  //draw line from player to follows
            ...
            case CamStates.Target:
             ...
             //   CinematicBars.Instance.ShowBars();
                ...
                lookDirection = followXform.forward;
        ...
        //Compensate Camera for walls
        CompensateForWalls(characterOffset, ref targetPosition);

        //Making a smooth transition between current state and disired state
