...
public class PoliceFOV : MonoBehaviour
{
    ...
    public static PoliceFOV enemyFOV;
    ...
    [HideInInspector] public NewMurphyMovement newMurphyMovement;
    ...
    private void Start()
    {
        ...
        patrol = GetComponent<PolicePatrol>();            // reference to the patrol script
        enemyAIController = GetComponent<PoliceAI>();
    ...
    private void Update()
    {
        ...
        playerPOS = Vector3.Distance(SceneSettings.Instance.humanPlayer.transform.position, this.transform.position);
            ...
            if (Vector3.Angle(transform.forward, dirToTarget) < viewAngle / 2) //check if target falls in view angle
            {
                ...
                if (!Physics.Raycast(transform.position, dirToTarget, dstToTarget, obstacleMask)) // if we dont collider with anything
                {
                    ...
                    if (fooledByCostume == false)        //If we are not fooled by the costume
                    {
                        if (playerFOV != false)
                        {
                            if (newMurphyMovement.IsHidden == false)
                            {
                                // checkState = true;
